  ...
  try {
    ...
    for (const cat of categories.slice(0, 10)) { // Show top 10 categories
      ...
      const [open, closed, resolved] = await Promise.all([
        prisma.market.count({ where: { category: categoryName, status: 'open' } }),
        prisma.market.count({ where: { category: categoryName, status: 'closed' } }),
        prisma.market.count({ where: { category: categoryName, status: 'resolved' } }),
      ]);
